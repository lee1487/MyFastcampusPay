...
import com.fastcampuspay.banking.application.port.in.*;
...
@Transactional
public class FirmbankingRequestService implements FirmbankingRequestUseCase, FindFirmbankingRequestUseCase {
    ...
    public void requestFirmbankingByEvent(RequestFirmbankingCommand command) {
        ...
        commandGateway.send(axonCommand).whenComplete((result, throwable) -> {
            if (throwable != null) {
                ...
                FirmbankingRequestJpaEntity requestedEntity = firmbankingRequestPort.createFirmbankingRequest(
                        new FirmbankingRequest.FromBankName(command.getFromBankName()),
                        new FirmbankingRequest.FromBankAccountNumber(command.getFromBankAccountNumber()),
                        new FirmbankingRequest.ToBankName(command.getToBankName()),
                        new FirmbankingRequest.ToBankAccountNumber(command.getToBankAccountNumber()),
                ...
                if (firmbankingResult.getResultCode() == 0){
                    // 성공
                    requestedEntity.setFirmbankingStatusValue(1);
                } else {
                    // 실패
                    requestedEntity.setFirmbankingStatusValue(2);
                }

                firmbankingRequestPort.modifyFirmbankingRequest(requestedEntity);
            }
        });
    }

    @Override
    public FirmbankingRequest findFirmbankingRequest(FindFirmbankingRequestCommand command) {
        FirmbankingRequestJpaEntity firmbankingRequest = firmbankingRequestPort.findFirmbankingRequest(new FirmbankingRequest.FirmbankingRequestId(command.getFirmbankingRequestId()));

        return firmbankingRequestMapper.mapToDomainEntity(firmbankingRequest, firmbankingRequest.getUuid());
