...
import com.fastcampuspay.remittance.adapter.out.persistance.RemittanceRequestJpaEntity;
import com.fastcampuspay.remittance.adapter.out.persistance.RemittanceRequestMapper;
...
public class RequestRemittanceService implements RequestRemittanceUseCase {
    ...
    public RemittanceRequest requestRemittance(RequestRemittanceCommand command) {
        ...
        if (command.getRemittanceType() == 0) {
            ...
            // 3-2. 외부 은행 계좌
            // 와부 은행 계좌가 적절한지 확인 (banking svc)
            // 법인 계좌 -> 외부 은행 계좌 펌뱅킹 요청 (banking svc)
